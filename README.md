...
一个轻量的 Chrome 浏览器扩展（Manifest V3），只用 `activeTab` 权限，不收集任何数据。同样可在 Microsoft Edge 浏览器中使用。
...
> 💡 同样适用于 Edge：本扩展基于 Manifest V3，在 Edge 中打开 `edge://extensions/`，开启「开发者模式」后，用同样的「加载已解压的扩展程序」方式加载即可。
